...
namespace vanilla {
	...
	TEST(serialization_builder_container_members_bid_request)
	{

		formats base = formats_builder()
            .type<Banner>()
              .member("width", &Banner::w)
              .member("height", &Banner::h)
              .register_container<std::list<int>>()
            //TODO: finish Banner
			.type<Impression>()
			  .member("id", &Impression::id)
              .member("banner", &Impression::banner)
              .register_container<boost::optional<Banner>>()
            //TODO: finish Impression
			.type<Site>()
			  .member("id", &Site::id)
			.type<BidRequest>()
			  .member("bid_request_id", &BidRequest::id)
			  .member("impression", &BidRequest::imp)
		...
		BidRequest br;
		auto encoded = to_json(br, fmt);
		BidRequest q = extract<BidRequest>(encoded, fmt);
		//ensure_eq(br, q);
	}
}
